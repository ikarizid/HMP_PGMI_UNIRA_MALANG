...
export default function AdminDashboard() {
  const stats = [
    { icon: Newspaper, label: 'Berita', value: beritaStore.getAll().length, color: '#3498DB', bg: 'rgba(52,152,219,0.08)', link: '/admin/berita' },
    { icon: Calendar, label: 'Event', value: eventStore.getAll().length, color: '#E89B1C', bg: 'rgba(232,155,28,0.08)', link: '/admin/event' },
    { icon: Image, label: 'Galeri', value: galeriStore.getAll().length, color: '#9B59B6', bg: 'rgba(155,89,182,0.08)', link: '/admin/galeri' },
    { icon: Users, label: 'Pengurus', value: strukturStore.getBPH().length + strukturStore.getDept().length, color: '#0D7C66', bg: 'rgba(13,124,102,0.08)', link: '/admin/struktur' },
    { icon: BookOpen, label: 'Jurnal', value: jurnalStore.getAll().length, color: '#E74C3C', bg: 'rgba(231,76,60,0.08)', link: '/admin/jurnal' },
  ];

  const recentBerita = beritaStore.getAll().slice(0, 3);
  const recentEvent = eventStore.getAll().slice(0, 3);
